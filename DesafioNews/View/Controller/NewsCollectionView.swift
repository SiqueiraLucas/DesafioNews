    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        if indexPath.section == 1{
            if indexPath.row == feedViewModel.countItems - 10{
                let endpoint = "https://mesa-news-api.herokuapp.com/v1/client/news?current_page=\(feedViewModel.returnCurrentPage())&per_page=\(feedViewModel.returnPerPage())&published_at="
                feedViewModel.request(endpoint: endpoint)
            }
        }
    }
